...
annotate_ensembl_transcripts <- function(x) {
  ensembl_ids <- rownames(x)

  stopifnot(all(stringr::str_starts(ensembl_ids,"ENST")))
  ...
  gene_annotation <- AnnotationDbi::select(
    ...
    columns=c("ENSEMBLTRANS","ENSEMBL","ENTREZID","GENENAME","SYMBOL")
  )

  # Since it is likely not a 1:1, select the lowest ENTREZID entry associated with ENSEMBL
  gene_annotation <- gene_annotation |>
    dplyr::mutate(rn = dplyr::row_number()) |>
    dplyr::group_by(.data$ENSEMBLTRANS) |>
    dplyr::slice_min(n=1, order_by=.data$ENTREZID, with_ties = FALSE) |>
    dplyr::arrange(rn) |>
    dplyr::select(-rn)
  ...
  # Now we need to verify rows match up
  stopifnot(all(ensembl_ids== gene_annotation$ENSEMBLTRANS))
